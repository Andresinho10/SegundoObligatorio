...
namespace SegundoObligatorioAlpha
{
    class Program
    {
                ...
                try
                {
                    switch (opcion)
                    {
                                        ...
                                        if (Buscar(lista, documento))
                                        {
                                            ...
                                            foreach (Cliente cliente in lista)
                                            {
                                                if (cliente.Cedula == documento)
                                                {
                                                    ...
                                                    Cliente customer = TraerCliente(documento, lista);                                                    
                                                    ...
                                                    { 	
                                                        ...
                                                        customer.AgregarTarjeta(card);
                                                        Console.WriteLine(card);
                                                    }
                                                    else if (opcionCredito.ToUpper() == "N")
                                                    {
                                                        Console.Write("\n\tPresione Enter para volver al Menú Principal");
                                                    }
                                                    else
                                    ...
                                    if (documento2.Length == 7 && int.TryParse(documento2, out aux3))
                                    {
                                        if (Buscar(lista, documento2))
                                        {
                                            ...
                                            foreach (Cliente cliente in lista)
                                            {
                                                ...
                                                {
                                                    Console.WriteLine(cliente);
                                                    Cliente customer = TraerCliente(documento2, lista);
                                                    Console.Write("\n\tDesea crear una nueva Tarjeta?(S/N): ");
                                                    ...
                                                    else if (opcionDebito.ToUpper() == "N")
                                                    {
                                                        Console.Write("\n\tPresione Enter para volver al Menú Principal");
                                                    }
                                                    else
                        ...
                        case "3":
                            ...
                            {
                                Cliente customer = TraerCliente(doc,lista);

                                if (customer.listaTarjetas.Count == 0)
                                    throw new Exception("\n\tEl cliente no tiene tarjetas a su nombre.");

                                foreach (Cliente cliente in lista)
                                    if (cliente.Cedula==doc)
                                    {
                                          foreach (Tarjeta tar in customer.listaTarjetas)
                                    {
                                        if (tar is Credito)
                                        {
                            ...
                            customer.BuscarTarjeta(opcionOperacion);
   
                            if (customer.BuscarTarjeta(opcionOperacion) is Credito)
                            {
                                Tarjeta a= customer.BuscarTarjeta(opcionOperacion);
                            ...
                            else if (customer.BuscarTarjeta(opcionOperacion) is Debito)
                           	
                            {                            	
                                Tarjeta b = customer.BuscarTarjeta(opcionOperacion);
                        ...
                        case "4":
                            ...
                            switch (opcion)
                            {
                                ...
                                case "2":
                                    ...
                                    {
                                        foreach(Tarjeta card in customer.listaTarjetas)
                                        {
                                            if (card is Credito)
                                        {
                                            Console.WriteLine(((Credito)card));
                                        }

                                        else
                                        {
                                            Console.WriteLine(((Debito)card));
                                        }
                                    }
                                ...
                                case "3":
                                    ...
                                    TraerTarjeta(numTarjeta, lista);

                                    Tarjeta cardList = TraerTarjeta(numTarjeta, lista);

                                    
                                    if(cardList is Credito)
                                    {
                                            if (((Credito)cardList).listaPagos.Count == 0 ) 
                                            {
                                               Console.WriteLine("\n\tNo hay pagos para listar.");	
                                                 ...
                                                 {
                                                     foreach(Pago pago in (((Credito)cardList).listaPagos))
                                                        {
                                                          Console.WriteLine("\n\t" + pago);
                                                        }
                                                 }
        ...
        //Devuelve un cliente de la lista buscandolo por nro de documento.
        static Cliente TraerCliente(string documento, List<Cliente> lista)
        {
            foreach (Cliente customer in lista)
            {
                if (customer.Cedula == documento)
                {
                    return customer;
                }
        ...
        //Devuelve una tarjeta buscandola por su nro. en la lista de tarjetas del cliente.
        static Tarjeta TraerTarjeta(int numeroDeTarjeta, List<Cliente> lista)
        {
            foreach (Cliente customer in lista)
            {
                foreach (Tarjeta card in customer.listaTarjetas )
                {
                    if (card.Numero == numeroDeTarjeta && card is Credito)
                    {
                        return ((Credito)card);
                    }

                    if (card.Numero == numeroDeTarjeta && card is Debito)
                        return ((Debito)card);
